...
89\% Recall@5 on internal benchmarks. We release ThotBook-AmentI as open-source
software, enabling fully private, local-first language model deployment.
\end{abstract}
...
Adam $\rightarrow$ L-BFGS recipe, grading each against an independent
reference (Table~\ref{tab:pinn}).
...
\midrule
Dirac (1+1 D)        & $3.9\times10^{-4}$ & $1.6\times10^{-6}$ & 37.6 & exact $\cos kx,\sin kx$ \\
Brans--Dicke (FLRW)  & $4.3\times10^{-3}$ & $4.2\times10^{-6}$ & 58.1 & exact Nariai power law \\
\textbf{Wheeler--DeWitt} & $\mathbf{1.8\times10^{-5}}$ & $2.4\times10^{-6}$ & 76.9 & Radau, \texttt{rtol}\,$10^{-12}$ \\
\bottomrule
\end{tabular}
\label{tab:pinn}
\end{table}
